...
@mcp.tool()
def batch_approve(
    min_confidence: Optional[float] = None, book_type: Optional[str] = None, max_count: int = 50, execute: bool = False
) -> dict:
    """
    Approve books matching filters.
    Set execute=True to apply (otherwise preview only).
    """
    return batch_approve_tool(min_confidence=min_confidence, book_type=book_type, max_count=max_count, execute=execute)
...
def batch_reject(
    ...
    return batch_reject_tool(
        book_type=book_type, max_confidence=max_confidence, reason=reason, max_count=max_count, execute=execute
    )
...
def audit(
    ...
    return get_audit_log(book_id=book_id, actor=actor, action=action, last_days=last_days, limit=limit)
