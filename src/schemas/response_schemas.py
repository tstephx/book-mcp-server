...
class SearchResponse(BaseModel):
    ...
    model_config = ConfigDict(
        json_schema_extra={"example": {"query": "docker networking", "results": [], "total_found": 5}}
    )
...
class ErrorResponse(BaseModel):
    ...
    model_config = ConfigDict(
        json_schema_extra={
            "example": {"error": "Book not found", "code": "BOOK_NOT_FOUND", "details": {"book_id": "invalid-uuid"}}
        }
    )
...
class SuccessResponse(BaseModel):
    ...
    model_config = ConfigDict(
        json_schema_extra={"example": {"success": True, "message": "Operation completed successfully", "data": {}}}
    )
