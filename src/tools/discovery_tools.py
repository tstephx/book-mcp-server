...
def register_discovery_tools(mcp: "FastMCP") -> None:
    ...
    def find_related_content(
        chapter_id: str = "", text_snippet: str = "", limit: int = 5, exclude_same_book: bool = True
    ) -> dict:
        ...
        try:
            if not chapter_id and not text_snippet:
                return {"error": "Provide either chapter_id or text_snippet", "results": []}
            ...
            with embedding_model_context() as generator:
                if chapter_id:
                    ...
                    source_info = (
                        f"Chapter {chapter['chapter_number']}: {chapter['title']} from '{chapter['book_title']}'"
                    )
                else:
            ...
            top_results = find_top_k(query_embedding, embeddings_matrix, k=limit * 3, min_similarity=0.2)
            ...
            return {"source": source_info, "results": results, "total_found": len(results)}
    ...
    def get_topic_coverage(
        ...
        Args:
            ...
            limit: Maximum number of books to return (default: 20, minimum: 1). Results
                   are sorted by average similarity, so highest-relevance books are returned first.
        ...
        try:
            ...
            sorted_books = sorted(books_coverage.values(), key=lambda x: x["avg_similarity"], reverse=True)
            sorted_books = sorted_books[:limit]

            returned_chapters = [c for b in sorted_books for c in b["chapters"]]
            logger.info(
                f"Topic '{topic}' covered in {len(sorted_books)} books, {len(returned_chapters)} chapters (of {len(all_results)} matched)"
            )
    ...
    @mcp.tool()
    def extract_code_examples(book_id: str, chapter_number: int, language: str = "") -> dict:
        ...
        try:
            ...
            if not chapter:
                return {"error": f"Chapter {chapter_number} not found in book {book_id}", "code_blocks": []}
